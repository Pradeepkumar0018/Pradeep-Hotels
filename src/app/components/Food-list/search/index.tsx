...
import FoodCard from '../../home/Food-list/Food-card';
...
export default function AdvanceSearch({ category }: { category?: string }) {
    ...
    const { LFood, updateFilter, filters } = useContext(FoodContext)!;
    ...
    const breadcrumbLinks = [
        { href: "/", text: "Home" },
        { href: "/LFood/LFoodlist", text: "Food List" },
    ];
    ...
    const filteredLFood = category
        ? LFood.filter((data: any) =>
            normalize(data.category) === normalize(category)
        )
        : LFood;

    // Sort logic
    const sortedLFood = [...filteredLFood].sort((a, b) => {
        const titleA = a.food_title?.toLowerCase() || "";
        const titleB = b.food_title?.toLowerCase() || "";
    ...
    const filteredCount = sortedLFood.length;

    return (
        <>
            <HeroSub
                title={filters?.category ? filters.category : "LFood List"}
                description="Explore a wide variety of delicious foods, bakery specials, restaurant favorites, and freshly prepared meals available near you."
                ...
                <div className='lg:max-w-screen-xl max-w-screen-md mx-auto'>
                    ...
                    <div className='lg:grid lg:grid-cols-12 gap-4'>
                        <div className='hidden lg:block lg:col-span-4'>
                            <div className='py-14 px-8 bg-white dark:bg-semidark shadow-Food rounded-lg'>
                                <p className='mb-6 text-2xl font-semibold'>Advanced Search</p>
                                <div className='flex flex-col gap-6'>
                                    {/* Map through keywords */}
                                    {searchData?.keywords?.map((option: any, index: any) => (
                        ...
                        <div className='col-span-12 lg:col-span-8'>
                            <div className="flex lg:flex-nowrap flex-wrap lg:gap-0 gap-6 w-full justify-between items-center pb-8">
                                <div className="flex w-full justify-between px-4 flex-1">
                                    <h5 className='text-xl '>{filteredCount} LFood Found</h5>
                            ...
                            {filteredLFood.length > 0 ?
                                <div className={` ${viewMode === 'grid' ? 'grid sm:grid-cols-2' : 'flex flex-col'} gap-6 px-4`}>
                                    {(sortOrder ? sortedLFood : LFood).map((data: any, index: any) => (
                                        <FoodCard key={index} Food={data} viewMode={viewMode} />
                                    ))}
                                </div>
                                :
