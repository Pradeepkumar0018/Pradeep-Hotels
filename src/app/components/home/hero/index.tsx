...
const Hero = () => {
  ...
  const [LFoodData, setLFoodData] = useState<any[]>([])
  const { LFood, updateFilter } = useContext(FoodContext)!;
  const [activeTab, setActiveTab] = useState("Bakery Foods");
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [location, setLocation] = useState("");
  const [error, setError] = useState('');
  const [foodIcon, setFoodIcon] = useState(1);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setFoodIcon((prev) => (prev === 4 ? 1 : prev + 1));
    }, 2000);
  ...
  const handleSearchBakeryFoods= () => {
    ...
    updateFilter('tag', 'Bakery Foods');
    router.push(`/LFood/LFoodlist`);
  };

  const handleSearchRestaurantFoods= () => {
    if (location.trim() === '') {
      setError('Please enter a location to search.');
      return;
    }
    setError('');
    updateFilter('location', location);
    updateFilter('tag', 'Restaurant Foods');
    router.push(`/LFood/LFoodlist`);
  };

  const suggestions = Array.from(new Set(LFoodData.map((item) => item.location)));
      ...
      <div className="container mx-auto lg:max-w-screen-xl md:max-w-screen-md relative z-10">
        <div className="grid lg:grid-cols-12 grid-cols-1">
          ...
          >
            <div className="mb-8">
              <h1 className="md:text-[50px] leading-[1.2] text-4xl  ml-4 text-midnight_text dark:text-white font-bold">
                Welcome to <span className="text-primary">Kholi Kitchen</span> - A Fan Love
              </h1>
            </div>
            <div className="max-w-xl ml-4 sm:w-full">
              <div className="flex gap-1 bg-trasperent">
                <button
                  className={`px-9 py-3 text-xl rounded-t-md focus:outline-none ${activeTab === "Bakery Foods"
                  ...
                  onClick={() => handleTabChange("Bakery Foods")}
                >
                  Bakery Foods
                </button>
                <button
                  className={`px-9 py-3 text-xl rounded-t-md focus:outline-none ${activeTab === "Restaurant Foods"
                    ? "bg-white dark:bg-darkmode dark:text-white text-midnight_text border-b border-primary"
                    : "text-midnight_text bg-white bg-opacity-50 dark:text-white dark:bg-darkmode dark:bg-opacity-50"
                    }`}
                  onClick={() => handleTabChange("Restaurant Foods")}
                >
                  Restaurant Foods
                </button>
              </div>
              <div className="bg-white dark:bg-transparent rounded-b-lg rounded-tr-lg">
                {activeTab === "Bakery Foods" && (
                  <div className="bg-white dark:bg-darkmode rounded-b-lg rounded-tr-lg shadow-lg p-8 pb-10">
                    <div className="relative rounded-lg border-0 my-2">
                      <div className="relative flex items-center">
                        <div className="absolute left-0 p-4">
                          <Image
                            src={getImgPath(`/images/svgs/food-${foodIcon}.svg`)}
                            alt="Icon"
                            height={24}
                            width={24}
                          />
                        ...
                        <input
                          type="text"
                          placeholder="Search Bakery Foods"
                          value={location}
                    ...
                    <div className="mt-6 flex flex-col-reverse gap-4 md:justify-between">
                      <div className="flex flex-col md:flex-row md:gap-4 w-full">
                        <button onClick={handleSearchBakeryFoods} className="flex-1 py-2 md:py-4 text-lg md:text-xl px-4 md:px-8 bg-primary text-white rounded-lg hover:bg-blue-700 transition duration-300 mb-2 md:mb-0 md:mr-2">
                          Search
                        </button>
                        <button onClick={handleSearchBakeryFoods} className="flex-1 py-2 md:py-4 text-lg md:text-xl px-4 md:px-8 bg-skyBlue/80 dark:bg-skyBlue/80 dark:hover:bg-skyBlue dark:hover:border-primary border border-transparent text-white rounded-lg hover:bg-skyBlue transition duration-300 text-nowrap">
                          Advance Search
                        </button>
                      </div>
                      {error && (
                ...
                {activeTab === "Restaurant Foods" && (
                  <div className="bg-white dark:bg-darkmode rounded-b-lg rounded-tr-lg shadow-lg p-8 pb-10">
                    ...
                    <div className="mt-6 flex flex-col-reverse gap-4 md:justify-between">
                      <div className="flex flex-col md:flex-row md:gap-4 w-full">
                        <button onClick={handleSearchRestaurantFoods} className="flex-1 py-2 md:py-4 text-lg md:text-xl px-4 md:px-8 bg-primary text-white rounded-lg hover:bg-blue-700 transition duration-300 mb-2 md:mb-0 md:mr-2">
                          Search
                        </button>
                        <button onClick={handleSearchRestaurantFoods} className="flex-1 py-2 md:py-4 text-lg md:text-xl px-4 md:px-8 bg-skyBlue/80 dark:bg-skyBlue/80 dark:hover:bg-skyBlue dark:hover:border-primary border border-transparent text-white rounded-lg hover:bg-skyBlue transition duration-300 text-nowrap">
                          Advance Search
                        </button>
                      </div>
                      {error && (
          ...
          <div className="lg:block hidden col-span-6 absolute xl:-right-60 right-0 bottom-0 -z-1">
            <Image
              src={getImgPath("/images/hero/Pradeep.png")}
              alt="heroimage"
              width={800}
              height={600}
